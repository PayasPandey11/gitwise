...
def mock_env(monkeypatch):
    ...
    monkeypatch.setattr(os.path, "expanduser", lambda x: "/home/user" if x == "~" else x) # Simplify expanduser
...
def test_load_config_local_exists(mock_file_open, mock_exists, mock_env):
    mock_exists.side_effect = lambda path: path == get_local_config_path() # Only local exists
    ...
    mock_file_open.assert_called_once_with(get_local_config_path(), "r", encoding="utf-8")

@patch("gitwise.config.os.path.exists")
@patch("builtins.open", new_callable=mock_open)
def test_load_config_global_exists_local_missing(mock_file_open, mock_exists, mock_env):
    mock_exists.side_effect = lambda path: path == get_global_config_path() # Only global exists
    dummy_config_data = {"llm_backend": "online", "openrouter_api_key": "key"}
    mock_file_open.return_value.read.return_value = json.dumps(dummy_config_data)

    loaded = load_config()
    assert loaded == dummy_config_data
    mock_file_open.assert_called_once_with(get_global_config_path(), "r", encoding="utf-8")
...
def test_config_exists_global_only(mock_exists, mock_env):
    ...
    assert config_exists() is True
    assert config_exists(local_only=False) is True # Explicitly check with local_only=False
...
def test_validate_config_valid():
    ...
    assert validate_config({"llm_backend": "online", "openrouter_api_key": "key"}) is True
...
def test_validate_config_invalid():
    ...
    assert validate_config({"llm_backend": "online"}) is False # Missing openrouter_api_key
...
@patch("gitwise.config.load_config")
def test_get_llm_backend_config_takes_precedence_over_env(mock_load_config_func, mock_env):
