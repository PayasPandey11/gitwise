...
def test_run_git_command_success(git_manager_instance, mock_subprocess_run):
    mock_subprocess_run.return_value = MagicMock(stdout="success", stderr="", returncode=0)
...
def test_run_git_command_failure_called_process_error(git_manager_instance, mock_subprocess_run):
    mock_subprocess_run.side_effect = subprocess.CalledProcessError(1, "git test-command", stderr="git error")
    with pytest.raises(RuntimeError) as excinfo:
        git_manager_instance._run_git_command(["test-command"])
    assert "Git command 'git test-command' failed with exit code 1." in str(excinfo.value)
...
def test_get_staged_files_success(git_manager_instance, mock_subprocess_run):
    mock_subprocess_run.return_value = MagicMock(stdout="M\tfile1.py\nA\tfile2.txt", returncode=0)
    files = git_manager_instance.get_staged_files()
    assert files == [("M", "file1.py"), ("A", "file2.txt")]
    mock_subprocess_run.assert_called_once_with(
        ["git", "diff", "--cached", "--name-status"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=False
    )
...
def test_get_unstaged_files_success(git_manager_instance, mock_subprocess_run):
    ...
    mock_subprocess_run.assert_called_once_with(
        ["git", "status", "--porcelain"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=False
    )
...
def test_get_staged_diff_success(git_manager_instance, mock_subprocess_run):
    ...
    mock_subprocess_run.assert_called_once_with(
        ["git", "diff", "--cached"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=False
    )
...
def test_get_file_diff_staged_success(git_manager_instance, mock_subprocess_run):
    ...
    assert diff == diff_content
    mock_subprocess_run.assert_called_once_with(
        ["git", "diff", "--cached", "--", "file1.py"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=False
    )

# Test get_changed_file_paths_staged
def test_get_changed_file_paths_staged_success(git_manager_instance, mock_subprocess_run):
    ...
    mock_subprocess_run.assert_called_once_with(
        ["git", "diff", "--cached", "--name-only"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=False
    )
...
def test_stage_files_success(git_manager_instance, mock_subprocess_run):
    ...
    mock_subprocess_run.assert_called_once_with(
        ["git", "add", "--", "file1.py", "file2.txt"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=False
    )
...
def test_stage_all_success(git_manager_instance, mock_subprocess_run):
    ...
    assert result is True
    mock_subprocess_run.assert_called_once_with(
        ["git", "add", "."], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=False
    )
...
def test_create_commit_success(git_manager_instance, mock_subprocess_run):
    ...
    assert result is True
    mock_subprocess_run.assert_called_once_with(
        ["git", "commit", "-m", "feat: test commit"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=False
    )

# Test get_current_branch
def test_get_current_branch_success(git_manager_instance, mock_subprocess_run):
    mock_subprocess_run.return_value = MagicMock(stdout="feature/branch-123\n", returncode=0)
    branch = git_manager_instance.get_current_branch()
    assert branch == "feature/branch-123"
    mock_subprocess_run.assert_called_once_with(
        ["git", "rev-parse", "--abbrev-ref", "HEAD"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=False
    )
...
def test_push_to_remote_simple(git_manager_instance, mock_subprocess_run):
    ...
    assert result is True
    mock_subprocess_run.assert_called_once_with(
        ["git", "push", "origin", "main"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=False
    )

def test_push_to_remote_with_remote_branch_and_force(git_manager_instance, mock_subprocess_run):
    mock_subprocess_run.return_value = MagicMock(returncode=0)
    result = git_manager_instance.push_to_remote(local_branch="feature/x", remote_branch="feature/x-upstream", remote_name="upstream", force=True)
    assert result is True
    mock_subprocess_run.assert_called_once_with(
        ["git", "push", "upstream", "feature/x:feature/x-upstream", "--force"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=False
    )

# Test get_default_remote_branch_name
def test_get_default_remote_branch_name_sym_ref(git_manager_instance, mock_subprocess_run):
    mock_subprocess_run.return_value = MagicMock(stdout="refs/remotes/origin/main\n", returncode=0)
    branch = git_manager_instance.get_default_remote_branch_name()
    assert branch == "main"
    mock_subprocess_run.assert_called_once_with(
        ["git", "symbolic-ref", "refs/remotes/origin/HEAD"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=True
    )

def test_get_default_remote_branch_name_remote_show(git_manager_instance, mock_subprocess_run):
    ...
    mock_subprocess_run.side_effect = [
        ...
        MagicMock(stdout="* remote origin\n  Fetch URL: ...\n  Push  URL: ...\n  HEAD branch: develop\n", returncode=0) # Second call for remote show
    ]
    branch = git_manager_instance.get_default_remote_branch_name()
    assert branch == "develop"
    expected_calls = [
        call(["git", "symbolic-ref", "refs/remotes/origin/HEAD"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=True),
        call(["git", "remote", "show", "origin"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=True)
    ]
    mock_subprocess_run.assert_has_calls(expected_calls)


def test_get_default_remote_branch_name_common_check(git_manager_instance, mock_subprocess_run):
    ...
    expected_calls = [
        call(["git", "symbolic-ref", "refs/remotes/origin/HEAD"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=True),
        call(["git", "remote", "show", "origin"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=True),
        call(["git", "show-ref", "--verify", "refs/remotes/origin/main"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=True)
    ]
    mock_subprocess_run.assert_has_calls(expected_calls)

def test_get_default_remote_branch_name_not_found(git_manager_instance, mock_subprocess_run):
...
def test_get_commits_between_success(git_manager_instance, mock_subprocess_run):
    ...
    mock_subprocess_run.assert_called_once_with(
        ["git", "log", "tag1..tag2", "--pretty=format:%H|%s|%an"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=False
    )

def test_get_commits_between_no_commits(git_manager_instance, mock_subprocess_run):
    mock_subprocess_run.return_value = MagicMock(stdout="", returncode=0) # Empty output
    commits = git_manager_instance.get_commits_between("tag1", "tag2")
    assert commits == []

# Test get_merge_base
def test_get_merge_base_success(git_manager_instance, mock_subprocess_run):
    mock_subprocess_run.return_value = MagicMock(stdout="commonancestorhash\n", returncode=0)
    base = git_manager_instance.get_merge_base("branch1", "branch2")
    assert base == "commonancestorhash"
    mock_subprocess_run.assert_called_once_with(
        ["git", "merge-base", "branch1", "branch2"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=True
    )
...
def test_has_staged_changes_true(git_manager_instance, mock_subprocess_run):
    ...
    mock_subprocess_run.assert_called_once_with(
        ["git", "diff", "--cached", "--quiet"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=False
    )
...
def test_has_unstaged_tracked_changes_true(git_manager_instance, mock_subprocess_run):
    ...
    mock_subprocess_run.assert_called_once_with(
        ["git", "diff", "--quiet"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=False
    )

# Test get_list_of_unstaged_tracked_files
def test_get_list_of_unstaged_tracked_files_present(git_manager_instance, mock_subprocess_run):
    mock_subprocess_run.return_value = MagicMock(stdout="file1.py\ndir/file2.js\n", returncode=0)
    files = git_manager_instance.get_list_of_unstaged_tracked_files()
    assert files == ["file1.py", "dir/file2.js"]
    mock_subprocess_run.assert_called_once_with(
        ["git", "diff", "--name-only"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=False
    )

# Test get_list_of_untracked_files
def test_get_list_of_untracked_files_present(git_manager_instance, mock_subprocess_run):
    mock_subprocess_run.return_value = MagicMock(stdout="untracked1.txt\nnew_dir/untracked2.log\n", returncode=0)
    files = git_manager_instance.get_list_of_untracked_files()
    assert files == ["untracked1.txt", "new_dir/untracked2.log"]
    mock_subprocess_run.assert_called_once_with(
        ["git", "ls-files", "--others", "--exclude-standard"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=False
    )

# Test get_local_base_branch_name
def test_get_local_base_branch_name_from_config(git_manager_instance, mock_subprocess_run):
    ...
    assert branch == "main"
    expected_calls = [
        call(["git", "config", "init.defaultBranch"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=True),
        call(["git", "rev-parse", "--verify", "main"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=False)
    ]
    mock_subprocess_run.assert_has_calls(expected_calls)

def test_get_local_base_branch_name_common_main(git_manager_instance, mock_subprocess_run):
...
def test_get_local_base_branch_name_common_master(git_manager_instance, mock_subprocess_run):
    ...
    expected_calls = [
        call(["git", "config", "init.defaultBranch"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=True),
        call(["git", "rev-parse", "--verify", "main"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=False),
        call(["git", "rev-parse", "--verify", "master"], cwd=MOCK_REPO_PATH, capture_output=True, text=True, check=False)
    ]
    mock_subprocess_run.assert_has_calls(expected_calls)

def test_get_local_base_branch_name_not_found(git_manager_instance, mock_subprocess_run):
    # All checks fail
    mock_subprocess_run.side_effect = [
        RuntimeError("config failed"), 
        MagicMock(returncode=1), # rev-parse main fails
