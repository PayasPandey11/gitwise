...
def generate_pr_description(commits: List[Dict]) -> str:
    ...
    # Guidance can be an empty string if not specifically needed here, or could be context dependent.
    prompt_text = PR_DESCRIPTION_PROMPT.format(
        commits=formatted_commits,
        repo_url=repo_url,
        repo_name=repo_name,
        guidance="" # Add specific guidance if needed, e.g., based on branch name or specific flags.
    )
    
    try:
        # The PR_DESCRIPTION_PROMPT is a single string, so pass it directly to get_llm_response
        description = get_llm_response(prompt_text) 
        if not description:
            raise ValueError("Empty PR description generated by LLM")
        return description
    except Exception as e:
        components.show_error(f"Could not generate PR description: {str(e)}")
        return ""
