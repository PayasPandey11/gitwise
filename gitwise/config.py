    ...
    for path in [local_path, global_path]:
        if os.path.exists(path):
            try:
                ...
                raise ConfigError(f"Config file at {path} is corrupt or invalid. Please re-run 'gitwise init'.")
    raise ConfigError("GitWise is not initialized in this repository. Please run 'gitwise init' first.")

def write_config(config: Dict, global_config: bool = False) -> str:
